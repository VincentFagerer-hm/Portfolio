...
const sketch = (p) => {
  ...
  const STICKER_URLS = [
    "Neo Brutalismus Formen/Union-1.png",
    "Neo Brutalismus Formen/Ellipse 6.png",
    "Neo Brutalismus Formen/Polygon 2.png",
    "Neo Brutalismus Formen/Star 4.png",
    "Neo Brutalismus Formen/Star 5.png",
    "Neo Brutalismus Formen/Star 6.png",
    "Neo Brutalismus Formen/Union-4.png",
    "Neo Brutalismus Formen/Union-2.png",
    "Neo Brutalismus Formen/Union-3.png",
    "Neo Brutalismus Formen/Union.png",
    "Neo Brutalismus Formen/Vector-1.png",
    "Neo Brutalismus Formen/Vector.png",
  ];
  ...
  p.draw = () => {
    ...
    p.background(255, 255, 230, TRAIL_ALPHA);
    ...
    for (let i = 0; i < particles.length; i++){
      ...
      // Schatten
      p.tint(17);
      p.image(pt.sprite, SHADOW_OFF, SHADOW_OFF, w, h);
  ...
  // Fake-Sticker (Fallback)
  function makeStickers(palette, outline, size){
    const arr = [], sw = outline;
    const gRect = (g, fill) => { g.background(255); g.stroke('#111'); g.strokeWeight(sw); g.fill(fill);
      g.rect(sw, sw, g.width - 2*sw, g.height - 2*sw, 8); };
    const gCircle = (g, fill) => { g.background(255); g.stroke('#111'); g.strokeWeight(sw); g.fill(fill);
      g.circle(g.width/2, g.height/2, g.width - 2*sw); };
    const gTriangle = (g, fill) => { g.background(255); g.stroke('#111'); g.strokeWeight(sw); g.fill(fill);
      const m=sw+6,w=g.width,h=g.height; g.triangle(m,h-m,w/2,m,w-m,h-m); };
    const gBolt = (g, fill) => { g.background(255); g.stroke('#111'); g.strokeWeight(sw); g.fill(fill);
      const w=g.width,h=g.height,m=sw+6; g.beginShape();
      g.vertex(w*0.35,m); g.vertex(w*0.55,m); g.vertex(w*0.45,h*0.44);
      g.vertex(w*0.70,h*0.44); g.vertex(w*0.28,h-m); g.vertex(w*0.48,h*0.60);
      g.vertex(w*0.30,h*0.60); g.endShape(p.CLOSE); };
    const gStar = (g, fill) => { g.background(255); g.stroke('#111'); g.strokeWeight(sw); g.fill(fill);
      const cx=g.width/2,cy=g.height/2,outer=g.width/2 - sw,inner=outer*0.45;
      g.beginShape(); for (let i=0;i<10;i++){ const ang=-p.HALF_PI + i*p.TAU/10; const r=(i%2===0)?outer:inner;
        g.vertex(cx+Math.cos(ang)*r, cy+Math.sin(ang)*r); } g.endShape(p.CLOSE); };
    const fns = [gRect, gCircle, gTriangle, gBolt, gStar];
    for (let i=0;i<10;i++){ const g=p.createGraphics(size,size); const fill=palette[i%palette.length]; fns[i%fns.length](g, fill); arr.push(g); }
    return arr;
  }
